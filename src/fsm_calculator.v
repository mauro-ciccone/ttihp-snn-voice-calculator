...
module fsm_calculator (
    ...
    input  wire [5:0] wta_spikes, // 6-channel one-hot spikes from WTA (Noise removed)
    ...
    // --- Internal Registers ---
    reg [1:0]  state;
    reg [2:0]  op1;       // Operand 1 (max value 4)
    ...
    // Indexes: 0=eis, 1=zwoi, 2=dru, 3=vier, 4=plus, 5=minus
    wire num_valid = wta_spikes[0] | wta_spikes[1] | wta_spikes[2] | wta_spikes[3];
    wire opr_valid = wta_spikes[4] | wta_spikes[5];

    wire [2:0] num_in = wta_spikes[3] ? 3'd4 :
                        wta_spikes[2] ? 3'd3 :
                        wta_spikes[1] ? 3'd2 :
                        wta_spikes[0] ? 3'd1 : 3'd0;

    // --- Combinational ALU ---
    // Signed arithmetic seamlessly creates 2's complement negatives for led_decoder
    ...
    always @(posedge clk_1mhz or negedge rst_n) begin
        if (!rst_n) begin
            ...
            timer    <= 13'd0;
            fsm_out  <= DISP_BLANK;
        end else if (tick_1ms) begin // Clock Enable: Executes logic strictly at 1 kHz
            
            // Advance timeout, cap at 5000 (5 seconds)
            ...
            case (state)
                ...
                WAIT_OPR: begin
                    if (opr_valid) begin
                        is_plus <= wta_spikes[4]; 
                        fsm_out <= wta_spikes[4] ? DISP_PLUS : DISP_MINUS;
                        state   <= WAIT_OP2;
                        timer   <= 13'd0;
                    end else if (timer == 13'd5000) begin
                        state   <= WAIT_OP1;
                ...
                WAIT_OP2: begin
                    if (num_valid) begin
                        fsm_out <= calc_res; // Combinational result routed instantly
                        state   <= SHOW_RES;
                        timer   <= 13'd0;
                    end else if (timer == 13'd5000) begin
                        state   <= WAIT_OP1;
